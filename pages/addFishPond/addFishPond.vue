<template>
	<view class="">
		...
		<view class="mainBody">
				...
				<button @click="submit(openid)">Submit</button>
				<uni-popup ref="fishSpeciesPopup"  background-color="#fff">
				    <uni-data-checkbox multiple v-model="fishSpeciesPopup.value" :localdata="fishSpeciesPopup.range" @change="changeCheckItem"></uni-data-checkbox>
					<button type="warn" @click="showDeleteFishPopup(fishSpeciesPopup.seletedFish)">删除选中鱼种</button>
					<button type="default" style="background-color: #1296db;color: #ffffff;" @click="showAddOtherFishPopup">添加鱼种</button>
				    <button type="primary" @click="changeFishSpecies(fishSpeciesPopup.seletedFish,fishSpeciesPopup.range)">确认</button>
				</uni-popup>
				<uni-popup ref="addFishPopup" type="dialog">
				    <uni-popup-dialog title="添加其他鱼种" mode="input" :before-close="true" @close="closeAddFish" @confirm="confirmAddFish"></uni-popup-dialog>
				</uni-popup>
				<uni-popup ref="deleteFishPopup" type="dialog">
				    <uni-popup-dialog title="温馨提示" content="确认删除所选鱼种？" mode="base"  @confirm="confirmDeleteFish(fishSpeciesPopup.seletedFish,fishSpeciesPopup.range)"></uni-popup-dialog>
				</uni-popup>
		</view>
	</view>
</template>
		...
		methods: {
			...
			async getFishSpeciesData(openid){
				...
				if(queryRes.code=='2000'){
					...
					this.fishes = fishes
					fishes = fishes.map((e,i)=>{return{text:e,value:i}})
					this.fishSpeciesPopup.range = fishes
				}else{
					let insertParams = [{
						report_type:'用户鱼种表',
						report_data:{openID:openid,fishSpecies:[]},
			...
			async confirmDeleteFish(seletedFish,range){
				...
				let updateParams = [{
					report_id:this.rid,
					report_data:{fishSpecies:fishes}
				}]	
			...
			async confirmAddFish(e){
				...
				let updateParams = [{
					report_id:this.rid,
					report_data:{fishSpecies:this.fishes}
				}]	
				let updateRes = await uni.$http.post('apiUpdate',updateParams)
				this.getFishSpeciesData(this.openid)			
